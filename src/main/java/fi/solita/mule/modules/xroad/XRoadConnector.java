 ...
 * {@see <a href="http://esuomi.fi/?mdocs-file=2268&mdocs-url=false">X-Road protocol for
 * adapter server messaging v4.0</a>} used in X-Road 6.0.
 * 
...
public class XRoadConnector {
    ...
    /**
     * Custom processor that sends xroad message. Optional values are taken from
     ...
     *         X-Road-serviceMemberClass, X-Road-serviceServiceVersion,
     *         X-Road-clientXroadInstance
     */
    @Processor
    public Object sendMessage(@Default("#[payload]") Object payload, @Optional String id,
            @Optional String clientXroadInstance, @Optional String clientMemberClass,
        ...
        try {
            for (Field field : headers.getClass().getDeclaredFields()) {
                if (!field.isSynthetic()) {
                    muleMessage.setOutboundProperty(X_ROAD_PROPERTY_PREFIX + field.getName(),
                            field.get(headers));
                }
